...
3. 调用 LLM 生成简洁标题（默认最多 6 个词）
4. 将 title 存储到 `ThreadState` 中（会被 checkpointer 持久化）

TitleMiddleware 会先将 LangChain message content 中的结构化 block/list 内容归一化为纯文本，再拼接到 title prompt 中，避免将 Python/JSON 的原始 repr 泄漏给标题生成模型。

## ⚠️ 重要：存储机制

### Title 存储位置

Title 存储在 **`ThreadState.title`** 中，而不是 thread metadata：
...
| 部署方式 | 是否持久化 | 说明 |
|---------|--------|------|
| **LangGraph Studio（本地）** | ❌ 否 | 仅存储在内存中，重启后会丢失 |
| **LangGraph Platform** | ✅ 是 | 自动持久化到数据库 |
| **自定义 + Checkpointer** | ✅ 是 | 需要配置 PostgreSQL/SQLite checkpointer |

### 如何启用持久化

如果希望本地开发时也能持久化 title，需要配置 checkpointer：
...
1. **读取方式不同**：Title 位于 `state.values.title`，而不是 `thread.metadata.title`
2. **性能考虑**：title 生成会增加约 0.5-1 秒延迟，可通过更快的模型优化
...
2. 检查日志：查找 `Generated thread title` 或相关错误信息
3. 确认是首次对话：只有 1 个用户消息和 1 个助手回复时才会触发

### Title 已生成但客户端看不到

1. 确认读取位置：应从 `state.values.title` 读取，而不是 `thread.metadata.title`
...
3. 查看数据库，确认 checkpointer 工作正常

## 架构设计

### 为什么使用 State 而不是 Metadata？

| 特性 | State | Metadata |
|------|-------|----------|
| **持久化** | ✅ 自动（通过 checkpointer） | ⚠️ 取决于实现 |
| **版本控制** | ✅ 支持时间旅行 | ❌ 不支持 |
| **类型安全** | ✅ 使用 TypedDict 定义 | ❌ 任意字典 |
| **可追溯** | ✅ 每次更新都会记录 | ⚠️ 只有最新值 |
