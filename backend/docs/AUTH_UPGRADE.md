...
首次启动不会在日志里打印随机密码，也不会写入默认 admin。这样可以避免启动日志泄露凭据，也避免在操作者创建账号前出现可被猜测的默认身份。

### 3. 创建 admin

访问 `http://localhost:2026/setup`，填写邮箱和密码创建第一个 admin 账号。创建成功后会自动登录并进入 workspace。

如果这是从无认证版本升级，创建 admin 后请再重启一次服务，让启动迁移把缺少 `user_id` 的历史 thread 归属到 admin。
...
其他用户可通过 `/login` 页面注册，自动获得 **user** 角色。每个用户只能看到自己的对话、上传文件、输出文件、memory 和自定义 agent。

## 安全机制

| 机制 | 说明 |
|------|------|
| JWT HttpOnly Cookie | Token 不暴露给 JavaScript，防止被 XSS 窃取 |
| CSRF Double Submit Cookie | 受保护的 POST/PUT/PATCH/DELETE 请求需携带 `X-CSRF-Token`；登录/注册/初始化/登出走 auth 端点 Origin 校验 |
| bcrypt 密码哈希 | 密码不会以明文存储 |
| Thread owner filter | `threads_meta.user_id` 由服务端认证上下文写入，搜索、读取、更新、删除默认按当前用户过滤 |
| 文件系统隔离 | 线程数据写入 `{base_dir}/users/{user_id}/threads/{thread_id}/user-data/`，在 sandbox 内统一映射为 `/mnt/user-data/` |
| Memory / agent 隔离 | 用户 memory 和自定义 agent 写入 `{base_dir}/users/{user_id}/...`；旧共享 agent 仅作为只读兼容回退 |
...
新随机密码会写入 `.deer-flow/admin_initial_credentials.txt`，文件权限为 `0600`。命令行只输出文件路径，不输出明文密码。

### 完全重置

删除统一 SQLite 数据库，重启后重新访问 `/setup` 创建新的 admin：
...
| `.deer-flow/admin_initial_credentials.txt` | `reset_admin` 生成的新凭据文件（0600，读取后应删除） |
| `.env` 中的 `AUTH_JWT_SECRET` | JWT 签名密钥（未设置时会自动生成并持久化到 `.deer-flow/.jwt_secret`，重启后 session 可保持） |

### 生产环境建议

```bash
# 生成持久化 JWT 密钥，避免重启后所有用户都需要重新登录
...
| `/api/v1/auth/initialize` | POST | 首次初始化第一个 admin（仅在无 admin 时可调用） |

## 兼容性

- **标准模式**（`make dev`）：完全兼容；无 admin 时访问 `/setup` 进行初始化
- **Gateway 模式**（`make dev-pro`）：完全兼容
- **Docker 部署**：完全兼容，`.deer-flow/data/deerflow.db` 需要持久化卷挂载
...
| 症状 | 原因 | 解决办法 |
|------|------|------|
| 启动后没看到密码 | 当前实现不会在启动日志中输出密码 | 首次安装请访问 `/setup`；忘记密码请使用 `reset_admin` |
| `/login` 自动跳转到 `/setup` | 系统还没有 admin | 在 `/setup` 创建第一个 admin |
| 登录后 POST 返回 403 | 缺少 CSRF token | 确认前端已更新 |
| 重启后需要重新登录 | `.jwt_secret` 文件被删除，且 `.env` 未设置 `AUTH_JWT_SECRET` | 在 `.env` 中设置固定密钥 |
