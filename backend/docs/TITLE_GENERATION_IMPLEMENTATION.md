...
- ✅ 实现 `_should_generate_title()`，用于检查是否需要生成标题
...
- ✅ 导入 `TitleMiddleware`
- ✅ 注册到 middleware 列表中：`[SandboxMiddleware(), TitleMiddleware()]`
...
| 方面 | State（✅ 已采用） | Metadata（❌ 未采用） |
|------|----------------|---------------------|
| **持久化** | 自动（通过 checkpointer） | 取决于实现，不够可靠 |
| **版本控制** | 支持时间旅行 | 不支持 |
...
**⚠️ 注意**：Title 位于 `state.values.title`，而不是 `thread.metadata.title`
